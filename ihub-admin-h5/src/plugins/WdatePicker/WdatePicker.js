...
(function () {
    ...
    A = $.$dpPath || J();
    if ($.$wdate) K(A + "/static/plugins/WdatePicker/skin/WdatePicker.css");
    V = Y;
    if ($.$crossFrame) {
        try {
    ...
    function I(G, _) {
        ...
        function E(K, J) {
            ...
            function C() {
                ...
                K.lang = _.name;
                K.skin = J.skin;
                var $ = ["<head><script>", "", "var doc=document, $d, $dp, $cfg=doc.cfg, $pdp = parent.$dp, $dt, $tdt, $sdt, $lastInput, $IE=$pdp.ie, $FF = $pdp.ff,$OPERA=$pdp.opera, $ny, $cMark = false;", "if($cfg.eCont){$dp = {};for(var p in $pdp)$dp[p]=$pdp[p];}else{$dp=$pdp;};for(var p in $cfg){$dp[p]=$cfg[p];}", "doc.oncontextmenu=function(){try{$c._fillQS(!$dp.has.d,1);showB($d.qsDivSel);}catch(e){};return false;};", "</script><script src=", A, "/static/plugins/WdatePicker/lang/", _.name, ".js charset=", _.charset, "></script>"];
                if (F) $[1] = "document.domain=\"" + I + "\";";
                for (var C = 0; C < D.length; C++)
                    if (D[C].name == J.skin) $.push("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + A + "/static/plugins/WdatePicker/skin/" + D[C].name + "/datepicker.css\" charset=\"" + D[C].charset + "\"/>");
                $.push("<script src=\"" + A + "/static/plugins/WdatePicker/calendar.js\"></script>");
...
module.exports.WdatePicker = WdatePicker;
...
(function(){
    ...
    /**
     * 将单个视频信息插入编辑器中(以网络地址方式插入选项)
     */
    function insertSingle(){
    	if("http" == NetVideoUrl.substr(0, 4)){
    		NetVideoUrl = '<iframe frameborder="0" width="640" height="498" src="'+ NetVideoUrl +'" allowfullscreen=""></iframe>';
    	}
    	editor.focus();  
		editor.execCommand('inserthtml' , NetVideoUrl);
...
//           switchSelect( ci ); ========================================================================== 2017-03-31 mupiao
         }
     }
    ...
    function createPreviewVideo(url){
		...
		if(url.substr(0, 4) == 'http'){
			$('#preview').html('<iframe frameborder="0" width="100%" height="100%" src="'+ url +'" allowfullscreen></iframe>');
		}else{
			$('#preview').html(NetVideoUrl);
			$('#preview').find('iframe, embed').attr({'width':'100%', 'height':'100%'});
		}
